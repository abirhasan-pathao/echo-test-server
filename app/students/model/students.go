...
type Student struct {
	gorm.Model
	Name  string `gorm:"not null" json:"name" validate:"required|minLen:2|maxLen:100|alphadotspace" message:"required:{field} is required|minLen:{field} must have at least 2 chars|maxLen:{field} can have at most 100 chars|alphadotspace:{field} can only contain letters, spaces, and dots"`
